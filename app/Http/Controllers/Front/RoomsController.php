...
class RoomsController extends Controller
{
	...
	public function delete_photo(Request $request )
	{
		...
		if($photos != NULL){
			$this->remove_photo_files($request->id, $photos->name);
			$photos->delete();
	...
	public function send_mail_owner($id){
		...
		if(!empty($rooms)){
			...
			$contact_user = User::find($rooms->user_id);
			$user = Auth::user()->id;	
			
			//Mail::to($contact_user->email)->send(new SendMailToOwner($user));
			Mail::to('iamirfanwebdeveloper@gmail.com')->send(new SendMailToOwner($user));
			
			$url = "/homes/$rooms->address_url/$rooms->id";
			
			
			$this->helper->flash_message('success', "Mail send to Owner"); // Call flash message function
