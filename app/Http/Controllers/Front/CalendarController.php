...
class CalendarController extends Controller
{
  ...
  public function ical_export(Request $request) {
    ...
    $result   = Calendar::where('room_id', $explode_id[0])->where('status','Not available')->groupBy('start_date','end_date')->get();
    ...
    $reservation = Calendar::where([
      ...
      $query->where([['status', 'Reserved'], ['source', 'Calendar']])
            ->orWhere([['status', 'Reserved'], ['source', 'Reservation']])
            ->orWhere('source', 'Import');
    })->first();

    if ($reservation === null) {
      // There is not Reservation data in BA.
      // Because the Reservation data of BA and VR are not the same.
      // So the VR Reservation data must be removed in VR database.
      $res = Reservation::where([
        ['room_id',$request->id],
        ['title', $season_name],
        ['checkin',$request->start_date],
        ['checkout',$request->end_date],
      ])->where(function($query) { $query->where([['status', '<>', 'Cancelled'], ['ba_status', 'Confirmed']]);
      })->delete();
      if ($res === null) {
        return json_encode(['success' => false]);
      }
    } else {
      // There is the Reservation data in BA.
      // delete reservation in the calendar table of BA.
      Calendar::where([
        ['room_id',$request->id],
        ['seasonal_name',$season_name],
        ['start_date', $request->start_date],
  ...
  public function ical_import(Request $request)
  {
    ...
    {      
      ...
      $price = RoomsPrice::find($request->id)->night;
      // Get events from IcalController
      
      $seasonal_name  = "iCal Res";
      for($i=0; $i<$ical->event_count; $i++)
      {
        ...
        $checkin = gmdate("Y-m-d", $start_date);
        $checkout = gmdate("Y-m-d", $end_date);
...
//     
  ...
  public function check_seasonal_conflict($season) {
    ...
    $rateId = 0;
    if(isset($data->rateId) && $data->rateId>0 ) $rateId = $data->rateId;

    $ba_roomid = Rooms::find($data->room_id)->ba_roomid;
    ...
    $rateData = [
      ...
      "firstNight" => $checkin,
      "lastNight" => $lastNight,
      "name" => $data->seasonal_name,
      "minNights" => $data->minimum_stay,
      "roomPrice" => $data->price,
      "extraPersonPrice" => @$data->additional_guest,
    ...
    if (BookingHelper::isAvailable()) {
      if($rateId>0) {
        ...
        if(isset($result[0]['success']) == false) {
          // var_dump($rateData);
          // var_dump($result);
          $msg = $result['error'];
          $errors = ['price' => [ 0 => $msg]];  
  ...
  public function check_unavailable_conflict($data) {
    ...
    if($reservation_id>0) {
      ...
      $reservationModel = new Reservation;
      $reservationModel->room_id = $request->id;
      $reservationModel->host_id = $room->user_id;
      $reservationModel->user_id = Auth::user()->id;
